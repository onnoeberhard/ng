...
for i = 1:m
    for k = 1:num_labels
        J_sum = J_sum - Y(k, i) * log(h(i, k)) - (1 - Y(k, i)) * log(1 - h(i, k));
    end
end
...
for j = 1:size(Theta1, 1)
    for k = 2:size(Theta1, 2)
        Theta1sqsu = Theta1sqsu + Theta1(j, k) ^ 2; 
    end
end
...
for j = 1:size(Theta2, 1)
    for k = 2:size(Theta2, 2)
        Theta2sqsu = Theta2sqsu + Theta2(j, k) ^ 2; 
    end
end

J = J_unreg + lambda / (2 * m) * (Theta1sqsu + Theta2sqsu);
